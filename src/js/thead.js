...
import {RegularHeadViewModel as RustRegularHeadViewModel} from "../../pkg";
 ...
 */
export class RegularHeaderViewModel extends RustRegularHeadViewModel {
    constructor(...args) {
        super(...args);
        for (const key of Reflect.ownKeys(RegularHeaderViewModel.prototype)) {
            this[key] = RegularHeaderViewModel.prototype[key];
        }
    }
        ...
        for (let d = 0; d < header_levels; d++) {
            ...
            if (d < header_levels - 1) {
                if (this._group_header_cache?.[d]?.[0]?.value === column_name) {
                    ...
                    th = this._draw_group_th(d, column_name);
    ...
    // clean() {
    //     this._clean_columns_cache(this._offset_cache);
    //     this._offset_cache = [];
